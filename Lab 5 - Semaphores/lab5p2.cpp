...
void* loggerThread(void *logfptr) {
	while(1) {
		char logBuffer[ENTRY_SIZE]; // placeholder array
		int len = deq(&buffer, logBuffer); // load from buffer into placeholder array
...
void writeLog(const char *format, ...) {
	...
	enq(&buffer, logBuffer, strlen(logBuffer) + 1);
}
